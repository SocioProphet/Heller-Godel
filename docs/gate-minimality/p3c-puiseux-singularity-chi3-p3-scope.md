...
It states that the exponent used by `chi_p` is a selected channel `beta`, not necessarily the leading term of the full local function.
...
Therefore, the manuscript does not define the A1 `chi_2` solely as local monodromy, solely as global curve monodromy, or solely as a code-level phase-character output. It defines the character by finite phase map from the selected Puiseux exponent channel. At A1, the selected exponent `1/2` also corresponds to the local square-root branch behavior, so the local square-root source and the finite `mu_2` phase source coincide.
...
The discriminant is not a square in `Q(x)` because it has odd-order zeros at `x=0` and `x=4/27`. Thus the Galois group over `Q(x)` is not contained in `A_3`; the generic cubic monodromy candidate is the full `S_3` rather than only its cyclic subgroup `A_3`.

However, `S_3` contains the normal cyclic subgroup `A_3 ~= Z/3`, generated by three-cycles permuting the three sheets. The character of this cyclic sheet-rotation subgroup acts naturally through `mu_3`. This is the strongest candidate analytic source for `chi_3`. P3.c closure must verify whether this is the correct identification and must not treat it as already closed by this scope.
...
over `Q(x)`, identify the `A_3 ~= Z/3` sheet-rotation subgroup inside the full `S_3` monodromy, and extract the sheet-permutation character as the candidate `mu_3` source.
