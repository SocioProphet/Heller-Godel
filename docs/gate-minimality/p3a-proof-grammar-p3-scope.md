...
This attachment is candidate-grade. This document does not treat `HG-FND-001` as settled Tier 1; its registry status remains `candidate; active core exists, registry normalization pending` until a separate normalization / promotion PR changes that status.

## 3. The A1 paradigm at p=2

The existing A1 / Catalan paradigm is documented at manuscript level and appendix level.

In `docs/manuscripts/calculus_invariant_characters_v2_1_3.md`, Section 2 defines the general type-theoretic framework as a simply typed lambda-calculus / implicational propositional fragment whose normal inhabitants are alpha-classes of eta-long beta-normal de Bruijn lambda terms. The canonical constructor statistic is:

```text
sigma_C(s)=#lambda-nodes + #application-nodes + #variable-leaves.
```

Section 3.2 then records the Catalan A1 grammar:
...
The load-bearing A1 paradigm is therefore type-theoretic: a type expression `C_A=(A -> A -> A) -> A -> A`, together with eta-long beta-normal inhabitants generated by `T ::= x | b(T,T)`, and counted by the fixed statistic `sigma_C`. The rooted binary-tree picture is a downstream visualization of those normal inhabitants, not the upstream grammar layer itself.
...
The executable arithmetic layer is not the proof grammar itself. `src/heller_godel/phase_characters.py` consumes finite arithmetic data derived downstream from the grammar, statistic, generating-function, and Puiseux layers, especially rational local exponents and finite phase indices.
...
At `p=3`, candidate canonical statistics include Fuss-Catalan numbers, ternary-constructor normal-form counts, or another structure to be determined in P3.a closure.

### R3 — Pipeline compatibility through downstream sub-obligations

The grammar must be specified so that P3.b, P3.c, and P3.d can be sequentially constructed against it.
...
The eventual rational Puiseux exponent data emerging from P3.c must be consumable by `src/heller_godel/phase_characters.py` at `p=3`, but this consumption is mediated by the downstream sub-obligations and is not a direct grammar-to-module interface requirement.

The arithmetic module expects downstream arithmetic inputs of the following forms:
...
For P3.a, this means the proof grammar must be sufficiently explicit for P3.b and P3.c to derive rational Puiseux exponent channels and finite phase data that are consumable by this module at `p=3`.
...
Because the A1 paradigm is type-theoretic, the primary P3.a candidates are stated at the type-and-term level. Combinatorial tree structures may still appear, but only as downstream visualizations or enumerator models of eta-long beta-normal inhabitants.

### Candidate G1: Type-level Fuss-Catalan analog
...
Candidate statistic: the canonical constructor statistic `sigma_C`, extended without changing level to count lambda nodes, application nodes, and variable leaves in the eta-long beta-normal representatives. A derived visualization may count ternary constructor nodes.

Candidate generating family: the Fuss-Catalan / ternary-constructor equation
...
Strength: this is the direct type-theoretic continuation of the A1 type `C_A=(A -> A -> A) -> A -> A`, one arity up.

Weakness: arity three, `Z/3`, `SU(3)`, and cubic-invariant structure are not automatically equivalent. P3.a closure must justify why this arity lift is the right type-level generator for the A2 path-beta sector rather than merely a convenient ternary analogy.

### Candidate G2: SU(3) chain-witness type grammar

Candidate form: a type expression encoding `SU(3)` chain data with `Z/3` monodromy selection represented in the type system. The exact type expression is not selected here.

A closure PR would need to inspect how the A1 chain witness is represented by the existing appendix and harness artifacts, then decide whether a type-level chain witness can be expressed as canonical normal inhabitants rather than as a purely analytic calibration family.

Candidate canonical statistic: chain length or a refined chain-character invariant, expressed as a statistic on the resulting canonical normal forms.

Strength: closer to the chain-family side of the existing witness apparatus.

Weakness: Appendix A.1 states that the A1 chain family is a null finite-character witness. Therefore an A2 chain-witness grammar cannot simply inherit nontrivial phase behavior; it must explain how the `p=3` chain type generates nontrivial finite phase data, if it does.

### Candidate G3: Cubic-invariant witness type grammar

Candidate form: a type expression or refinement-typed grammar encoding triples `(v1,v2,v3)` with `Omega`-preservation under an active-sector group action, for example as a dependent or refinement condition of the form:

```text
Omega(v1,v2,v3) preserved under admissible active-sector morphisms.
```

Candidate canonical statistic: an `Omega`-related invariant of the typed triple, lifted to a statistic on canonical normal forms.

Strength: most directly bound to `HG-MTH-011` condition (v), the cubic invariant `Omega`.

Weakness: this form is closest to the A2 active-sector geometry but farthest from the established Catalan type-expression paradigm. P3.a closure must show how its normal inhabitants lead to a proof-family generating function and Puiseux singularity channel rather than only encoding an invariant constraint.
...
6. Does not promote `HG-FND-001` from candidate to settled Tier-1 status. `HG-FND-001` normalization is a separate open obligation, not in P3.a scope.
7. Does not authorize any Heller-Einstein PR. HE mediation is named as available but its use requires separate authorization.
8. Does not extend to `A_n` for `n >= 3`.
9. Does not cross into `SocioProphet/yang-mills`. The proof-character pipeline is HG-internal.
