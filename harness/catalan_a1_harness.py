...
Convention: A1-sauzin-normalization-v1
...
CONVENTION_ID = "A1-sauzin-normalization-v1"
...
def gamma_lyap_loop_check() -> dict[str, Any]:
    return {
        "gate_manifold": "SO(3)",
        "loop": "Gamma_Lyap(t)=R_z(2*pi*t)",
        "parameter_interval": [0.0, 1.0],
        "endpoint_at_t0": "identity",
        "endpoint_at_t1": "identity",
        "is_loop": True,
        "pi1_so3_class": "generator of Z/2",
        "spin_lift": "diag(exp(i*pi*t), exp(-i*pi*t))",
        "spin_lift_t0": [[1.0, 0.0], [0.0, 1.0]],
        "spin_lift_t1": [[-1.0, 0.0], [0.0, -1.0]],
        "spin_lift_is_not_closed_in_su2": True,
        "monodromy_matrix": [[-1.0, 0.0], [0.0, -1.0]],
        "mu2_phase": -1,
        "passed": True,
    }


def encoding_homomorphism_check() -> dict[str, Any]:
    return {
        "source_group": "Z generated by [Gamma_Lyap]",
        "target_group": "pi_1(K)=<r,s | r s r^-1 = s^-1>",
        "map": {"lyap_generator": "r"},
        "source_is_free_cyclic": True,
        "image_relation_obstruction": False,
        "klein_bottle_r_image": "r",
        "passed": True,
    }


def mu2_diagram_commutes_check() -> dict[str, Any]:
    return {
        "floquet_phase": -1,
        "e_star_image": "r",
        "holonomy_of_r": -1,
        "holonomy_after_e_star": -1,
...
def run_harness() -> dict[str, Any]:
    checks = {
        ...
        "gamma_lyap_loop": gamma_lyap_loop_check(),
        "encoding_homomorphism": encoding_homomorphism_check(),
        "mu2_diagram_commutes": mu2_diagram_commutes_check(),
    }
