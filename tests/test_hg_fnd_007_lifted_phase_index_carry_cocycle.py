...
def test_mu2_canonical_nontrivial_carry():
    assert carry(1, 1, 2) == 1


def test_mu2_zero_plus_zero_has_zero_carry():
    assert carry(0, 0, 2) == 0


def test_mu2_carry_cocycle_identity_all_residues():
    for a in range(2):
        for b in range(2):
            for c in range(2):
...
def test_tame_symbol_is_structurally_distinct_from_carry():
...
def test_mu3_carry_table_is_verified_matrix():
